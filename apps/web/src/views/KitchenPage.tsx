...
export function KitchenPage() {
  const openKitchenCards = demo.orders
    .filter((o) => o.status === "open")
    .map((o) => ({
      order: o,
      lines: o.lines.filter((l) => l.stationId === "kitchen" && l.status !== "done" && l.status !== "canceled")
    }))
  ...
  return (
    <main className="mx-auto max-w-5xl p-4">
      <div className="flex items-baseline justify-between">
        ...
        <div className="text-sm text-slate-400">{openKitchenCards.length} active</div>
      </div>

      <div className="mt-4 grid gap-3 md:grid-cols-2">
        {openKitchenCards.length === 0 ? (
          <div className="text-sm text-slate-400">No kitchen items.</div>
        ) : (
          openKitchenCards.map(({ order, lines }) => (
